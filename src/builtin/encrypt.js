 ...
 */
export async function encrypt(action, baseDir, vars, _) {
  const functionName = BUILTIN_PREFIX + 'encrypt';
  ...
  if (askEncryptionPassword) {
    ...
    let confirmEncryptionPassword = await inputPassword('Confirm encryption password: ');

    while (encryptionPassword !== confirmEncryptionPassword) {
      encryptionPassword = await inputPassword();
      confirmEncryptionPassword = await inputPassword('Confirm encryption password: ');
      if (encryptionPassword !== confirmEncryptionPassword) {
        console.log(chalk.red('Passwords do not match. (press Control-C to exit)'));
      }
    }
  }
  ...
  await encryptFile(baseDir, src, encryptionPassword);
}
